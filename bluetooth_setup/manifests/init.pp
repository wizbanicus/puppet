...
# We are using a hash of mice and keyboards along with host names, this will set defaults in the files
...
#
class bluetooth_setup {
# bt1 and bt2 are bluetooth device 1 and bluetoth device 2 (not sure which is mouse and which is keyboard)
  $my_hosts = { 
...
$bt_prefs_bh_ad="/Users/Administrator/Library/Preferences/ByHost/com.apple.Bluetooth.${sp_platform_uuid}.plist"
  ...
  file {"${bt_prefs}" :
    content => template("bluetooth_setup/com.apple.Bluetooth.plist.erb"),
    ensure  => file,
    owner   => 'root',
    group   => 'wheel',
    mode    => 700,
  }
  exec {"/usr/bin/plutil -convert binary1 ${bt_prefs}" :
    subscribe   => File["${bt_prefs}"],
    refreshonly => true,
  }
...
# as we are setting up the template user NOT clubhouse directly.
  ...
  file {"${bt_prefs_bh_ch}":
    content => template("bluetooth_setup/com.apple.Bluetooth.HOSTNAME.plist.erb"),
    ensure  => file,
    owner   => 'root',
    mode    => 700,
  }
  exec {"/usr/bin/plutil -convert binary1 ${bt_prefs_bh_ch}":
    subscribe   => File["${bt_prefs_bh_ch}"],
    refreshonly => true,
  }
  file {"${bt_prefs_bh_ad}":
    content => template("bluetooth_setup/com.apple.Bluetooth.HOSTNAME.plist.erb"),
  ...
  exec {"/usr/bin/plutil -convert binary1 ${bt_prefs_bh_ad}":
    subscribe   => File["${bt_prefs_bh_ad}"],
    refreshonly => true,
  }
}
}
